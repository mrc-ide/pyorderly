...
def test_read_simple_trivial_parameters():
    assert _read_py("orderly.parameters()") == {"parameters": {}}
    a = _read_py("orderly.parameters(a=None)")
    assert a == {"parameters": {"a": None}}
    ab = _read_py("orderly.parameters(a=None, b=1)")
...
def test_prevent_complex_types_in_parameters():
    ...
    with pytest.raises(Exception, match=re.escape(msg)):
        _read_py("orderly.parameters(a=len)")
